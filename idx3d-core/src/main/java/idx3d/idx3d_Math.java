...
// Static class for accelerated mathematical operations
{	
	...
	// R A N D O M  N U M B E R S
		...
		public static final float random(float scale)
		{
			return scale*random();
		}
		...
		{
			return a+(1-cos(d*pi))*0.5f*(b-a);
		}
		
		public static final float fractalNoise(float rel, int iterations)
		{
			float result=0;
			for (int i=0;i<iterations;i++)
				result+=randomNoise(rel,1<<(i+2)) / (1<<i);
			return crop(result,-1,1);
		}
		

		public static final float randomNoise(float rel, float density)
		{
			float scaled=rel*density;
			int floor=(int)scaled;
			return interpolate(rnd(floor),rnd(floor+1),scaled-floor);
		}
		
		public static float rnd(int seed)
		// returns a random number between -1 and 1
		{
    			int n =(seed<<13)^seed;
			return 1f- ((n * (n * n * 15731 + 789221) + 1376312589)&0x7FFFFFFF)*0.000000000931322574615478515625f;
		}
}
