...
// used to create instances of idx3d_Object for given object types
{
	...
	public static idx3d_Object createSphere(int iterations)
	{
		idx3d_Object obj=new idx3d_Object();
		float alpha=(float)(pi*2/3);
		
		obj.addVertex(new idx3d_Vertex(1,0,0));
		obj.addVertex(new idx3d_Vertex((float)Math.cos(alpha),(float)Math.sin(alpha),0));
		obj.addVertex(new idx3d_Vertex((float)Math.cos(alpha),(float)Math.sin(-alpha),0));
		obj.addVertex(new idx3d_Vertex(0,0,1));
		obj.addVertex(new idx3d_Vertex(0,0,-1));
		obj.rebuild();
		
		obj.addTriangle(1,0,3);
		obj.addTriangle(2,1,3);
		obj.addTriangle(0,2,3);
		obj.addTriangle(0,1,4);
		obj.addTriangle(1,2,4);
		obj.addTriangle(2,0,4);
		
		obj.rebuild();
		for (int i=0;i<obj.vertices();i++) obj.vertex(i).pos.normalize();
		
		for (int i=0;i<iterations; i++)
		{
			int num=obj.triangles();
			for (int j=0;j<num;j++)
			{
				idx3d_Triangle t=obj.triangle(j);
				idx3d_Vertex v4=new idx3d_Vertex(idx3d_Vector.add(t.p1.pos,t.p2.pos).normalize());
				idx3d_Vertex v5=new idx3d_Vertex(idx3d_Vector.add(t.p2.pos,t.p3.pos).normalize());
				idx3d_Vertex v6=new idx3d_Vertex(idx3d_Vector.add(t.p3.pos,t.p1.pos).normalize());
				
				obj.addVertex(v4);
				obj.addVertex(v5);
				obj.addVertex(v6);
				
				obj.addTriangle(v4,t.p2,v5);
				obj.addTriangle(v6,v5,t.p3);
				obj.addTriangle(v4,v5,v6);
	...
	public static idx3d_Object spherePatch(int detail, float rad)
	{
		int steps=4*detail;
		idx3d_Object obj=new idx3d_Object();
		
		float dAngle=(float)(2*pi/(float)steps);
		float alpha,beta;
		idx3d_Vertex[][] v=new idx3d_Vertex[steps][steps];
		...
		for (int b=0;b<steps;b++)
		{
			...
			{
				alpha=(float)a*dAngle;
				r2=(float)Math.abs(rad*idx3d_Math.cos(beta));
				r2=rad*idx3d_Math.cos(beta);
				x=r2*idx3d_Math.cos(alpha);
				y=rad*idx3d_Math.sin(beta);
				z=r2*idx3d_Math.sin(alpha);
				v[a][b]=new idx3d_Vertex(x,y,z);
				obj.addVertex(v[a][b]);
			}
		}
		...
		for (int b=0;b<steps;b++)
		{
			for (int a=0;a<steps;a++)
			{
				p1=v[a%steps][b%steps];
				p2=v[(a+1)%steps][b%steps];
				p3=v[(a+1)%steps][(b+1)%steps];
				p4=v[a%steps][(b+1)%steps];
