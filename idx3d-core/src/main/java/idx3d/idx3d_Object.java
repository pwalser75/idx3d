...
public class idx3d_Object extends idx3d_CoreObject
{
	// F I E L D S
		...
		public idx3d_Vertex[] vertex;
		public idx3d_Triangle[] triangle;
	...
	// D A T A  S T R U C T U R E S
		...
		public idx3d_Object setMaterial(idx3d_Material m)
		{
			material=m;
			return this;
		}
		...
		public void rebuild()
		{
			...
			enumeration=vertexData.elements();
			for (int i=vertices-1;i>=0;i--) vertex[i]=(idx3d_Vertex)enumeration.nextElement();
			
			// Generate faster structure for triangles
			triangles=triangleData.size();
			triangle=new idx3d_Triangle[triangles];
			enumeration=triangleData.elements();
			for (int i=triangles-1;i>=0;i--)
			{
				triangle[i]=(idx3d_Triangle)enumeration.nextElement();
				triangle[i].id=i;
			}
		...
		public idx3d_Object getClone()
		{
			...
			for(int i=0;i<triangles;i++) 
				obj.addTriangle(triangle[i].p1.id,triangle[i].p2.id,triangle[i].p3.id);
		...
		public void removeDuplicateVertices()
		{
			...
			Enumeration enumeration=edgesToCollapse.elements();
			while(enumeration.hasMoreElements()) edgeCollapse((idx3d_Edge)enumeration.nextElement());
		...
		public idx3d_Object flipNormals()
		{
			rebuild();
			for (int i=0;i<triangles;i++)
				triangle[i].flipNormal();
			regenerate();
			return this;
		}
	
	public void fractalNoise(float factor, int iterations)
	{
		rebuild();
		idx3d_Vector max=max();
		idx3d_Vector min=min();
		float size=factor*getDimension().length();
		System.out.println(size);
		
		for (int i=0;i<vertices;i++)
		{
			vertex[i].pos.x+=factor*idx3d_Math.fractalNoise((vertex[i].pos.x-min.x)/(max.x-min.x),iterations);
			vertex[i].pos.y+=factor*idx3d_Math.fractalNoise((vertex[i].pos.y-min.y)/(max.y-min.y),iterations);
			vertex[i].pos.z+=factor*idx3d_Math.fractalNoise((vertex[i].pos.z-min.z)/(max.z-min.z),iterations);
		}
	}
