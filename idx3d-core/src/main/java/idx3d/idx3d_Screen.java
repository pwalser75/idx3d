...
// defines a virtual screen which is a server for rendered images
{
	...
	// C O N S T R U C T O R S
		...
		public idx3d_Screen(idx3d_Texture t)
		{
			width=t.width;
			height=t.height;
			this.w=width;
	...
	// P U B L I C   M E T H O D S
		...
		{
			DataBufferInt buffer=new DataBufferInt(pixel,pixel.length);
	...
	// IMAGE OVERLAYING
		...
		public void drawSprite(idx3d_Texture texture, int posx, int posy, int xsize, int ysize)
		{
			drawSprite(p,w,h,texture,posx,posy,xsize,ysize);
		}
		...
		public void drawBackground(idx3d_Texture texture,  int xsize, int ysize)
		{
			if ((texture.width==w) && (texture.height==h))
				System.arraycopy(texture.pixel,0,pixel,0,w*h);
			else draw(p,w,h,texture,0,0,xsize,ysize);
		}		
	...
	// Private part of image overlaying
		...
		private void drawSprite(int[] buffer, int width, int height, idx3d_Texture texture, int posx, int posy, int xsize, int ysize)
		{
			if (xsize==0) return;
			if (ysize==0) return;
			...
			for(int j=yBase;j<yend;j++)
			{
				...
				for(int i=xBase;i<xend;i++)
				{
					newpixel=texture.pixel[(tx>>8)+offset2];
					if ((newpixel&0xFF000000)!=0) buffer[i+offset1]=newpixel;
					tx+=dtx;
				}
				ty+=dty;
			}
		...
		public void bilinearDraw(idx3d_Texture t)
		{
			int w=t.width;
			int h=t.height;
			int pos=0;
			...
			for (int ydest=0; ydest<height; ydest++)
				for (int xdest=0; xdest<width; xdest++)
				{
					...
					xrel=x-xpos;
					yrel=y-ypos;
					colorA=t.pixel[xpos+ypos*t.width];
					colorB=t.pixel[xpos+1+ypos*t.width];
					colorC=t.pixel[xpos+(ypos+1)*t.width];
					colorD=t.pixel[xpos+1+(ypos+1)*t.width];
					weightA=(int)(255*(1-xrel)*(1-yrel));
					weightB=(int)(255*xrel*(1-yrel));
					weightC=(int)(255*(1-xrel)*yrel);
					weightD=(int)(255*xrel*yrel);
					pixel[pos++]=idx3d_Color.add(
						idx3d_Color.scale(colorA,weightA),idx3d_Color.add(
							idx3d_Color.scale(colorB,weightB),idx3d_Color.add(
								idx3d_Color.scale(colorC,weightC),
									idx3d_Color.scale(colorD,weightD))));
				}
		}
