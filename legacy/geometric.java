...
public final class geometric extends Applet implements Runnable
{
	...
	public void init()
	{
		...
		// BUILD SCENE
			...
			scene.addMaterial("default",new idx3d_Material(getDocumentBase(),"chrome.material"));
			...
			try{
				new idx3d_3ds_Importer().importFromURL(new java.net.URL(getDocumentBase(),"export.3ds"),scene);
				scene.rebuild();
				for (int i=0;i<scene.objects;i++) scene.object[i].setMaterial(scene.material("default"));
				scene.normalize();
							}
