...
public final class idx3dMaterialLab extends Frame
{
	...
	{
		File newFile=browseForFile("Read Scene from 3ds File",".3ds",true);
	...
	{
		File newFile=browseForFile("Import Texture",".jpg",true);
		if (newFile==null) return;
	...
	{
		File newFile=browseForFile("Import Envmap",".jpg",true);
		if (newFile==null) return;
	...
	private File browseForFile(String dialogTitle, String fileType, boolean mustExist)
	{
		String[] fileTypes=new String[1];
		fileTypes[0]=fileType;
		return browseForFile(dialogTitle, fileTypes, mustExist);
	}
	
	private File browseForFile(String dialogTitle, String[] fileTypes, boolean mustExist)
	{
		FileDialog fd = mustExist? new FileDialog(this, dialogTitle, FileDialog.LOAD) :  new FileDialog(this, dialogTitle, FileDialog.SAVE);
		fd.setTitle(dialogTitle);
		if (lastDirectory!=null) fd.setDirectory(lastDirectory);
		fd.setFilenameFilter(new FileTypeFilter(fileTypes));
		fd.show();
	...
	{
		File file=browseForFile("Save Material",".material",false);
		if (file==null) return;
		int id;
		try
		{
	...
	{
		File file=browseForFile("Read Material",".material",true);
		if (file==null) return;
