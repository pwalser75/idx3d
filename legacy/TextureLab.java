...
try{
            ...
            java.awt.image.BufferedImage buf = new java.awt.image.BufferedImage(texture.width,texture.height,java.awt.image.BufferedImage.TYPE_INT_BGR);
 Graphics g = buf.createGraphics();
 g.drawImage(textureView.getImage(),0,0,null);
 
           java.io. FileOutputStream fout = new java.io.FileOutputStream(new java.io.File("generated.jpg"));
          com.sun.image.codec.jpeg.JPEGEncodeParam jepar = com.sun.image.codec.jpeg.JPEGCodec.getDefaultJPEGEncodeParam(buf);
            jepar.setQuality( 1.0f, true);
            
            com.sun.image.codec.jpeg.JPEGImageEncoder jpege = com.sun.image.codec.jpeg.JPEGCodec.createJPEGEncoder(fout) ;
            jpege.encode(buf, jepar);
            fout.close();                       
           
            } catch (java.io.IOException ioe) { System.err.println(ioe.getMessage());
            }
	...
	private void generateTexture()
	{
		...
		{
			modeId=1;
			texture=idx3d_TextureFactory.colorize(
				idx3d_TextureFactory.PERLIN(w,h, p, d, s, 1024),
		...
		if (wave.getState())
		{
			modeId=2;
			texture=idx3d_TextureFactory.colorize(
				idx3d_TextureFactory.WAVE(w,h, p, d, s, 1024),
				idx3d_Color.makeGradient(colors,1024));
		}
		if (grain.getState())
		{
			modeId=3;
			texture=idx3d_TextureFactory.colorize(
				idx3d_TextureFactory.GRAIN(w,h, p, d, s,8,1024),
				idx3d_Color.makeGradient(colors,1024));
		}
