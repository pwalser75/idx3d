...
public final class demo7 extends Applet implements Runnable
{
	...
	public void init()
	{
		...
		// BUILD SCENE
			...
			idx3d_Texture stone1=idx3d_TextureFactory.bilinearResample(new idx3d_Texture(getDocumentBase(),"textures/stone5.jpg"),512,512);
			scene.addMaterial("Stone1", new idx3d_Material(stone1));
			scene.addMaterial("Stone2", new idx3d_Material(new idx3d_Texture(getDocumentBase(),"textures/stone4.jpg")));
			...
			try{
				new idx3d_3ds_Importer().importFromURL(new java.net.URL(getDocumentBase(),"meshes/venus.3ds"),scene);
				scene.normalize();
